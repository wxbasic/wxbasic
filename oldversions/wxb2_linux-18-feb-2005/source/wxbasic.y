...
{
    return wOpNode( W_OP_COMMA, 0, node1, node2 );
}
...
%type <node> optArrayValueList
%type <node> arrayValueList
%type <node> lval
...
command:
    ...
    | W_TOKEN_FUNCTION_NAME W_TOKEN_EQ expr
        {   if ($1 != wCurrentScope || !wInStack(wBlockStack,W_TOKEN_FUNCTION)) {
            ...
            $$ = wOpNode( W_OP_RETURN_SET_VALUE, 1, $3, NULL ); }
    ...
    | W_TOKEN_FUNCTION_NAME '('
        ...
        {   int args = wPopStack( wArgCountStack );
            ...
            $$ = wSymbolNode( W_OP_FUNCTION_CALL, $1, 0, $4, NULL ); }
    ...
    | W_TOKEN_FUNCTION_NAME sep
        ...
        $$ = wSymbolNode( W_OP_FUNCTION_CALL, $1, 0, NULL, NULL ); }
    ...
    | W_TOKEN_BUILTIN_NAME '('
        ...
        { int args = wPopStack( wArgCountStack );
          wCheckArgCount( $1, args );
          $$ = wSymbolNode( W_OP_BUILTIN_CALL, $1, 0, $4, NULL ); }

    | W_TOKEN_BUILTIN_NAME sep
        { if ($1->args != 0) {
              wExpected("'('");
          }
          $$ = wSymbolNode( W_OP_BUILTIN_CALL, $1, 0, NULL, NULL ); }

    | W_TOKEN_CLOSE fileHandle sep
        {   $$ = wOpNode( W_OP_CLOSE, 0, $2, NULL ); }

    | W_TOKEN_CLOSE sep
        {   $$ = wOpNode( W_OP_CLOSE, 0, NULL, NULL ); }
    ...
    | W_TOKEN_CONTINUE sep
        {   if (!wInStack(wBlockStack,W_TOKEN_FOR) && !wInStack(wBlockStack,W_TOKEN_WHILE)) {
            ...
            $$ = wOpNode( W_OP_CONTINUE, 0, NULL, NULL ); }

    | W_TOKEN_DELETE expr sep
        { $$ = wOpNode( W_OP_DELETE, 0, $2, NULL ); }
    ...
    | W_TOKEN_IF
      ...
      expr W_TOKEN_THEN sep block elseIf sep
        {   wPopStack(wBlockStack);
            $$ = wOpNode( W_OP_IF, 0, wOpNode( W_OP_IF_TRUE, 0, $3, $6 ), $7 ); }
    ...
    | W_TOKEN_FOR W_TOKEN_EACH W_TOKEN_VARIABLE_NAME W_TOKEN_IN W_TOKEN_ARRAY_NAME optBrackets
        ...
        sep block
        {   wPopStack(wBlockStack);
            $$ = wSymbolNode( W_OP_FOR_EACH, $3, 0,
                wSymbolNode( W_OP_NOOP, $5, 0, NULL, NULL ), $9 ); }
    ...
    | W_TOKEN_FOR W_TOKEN_EACH W_TOKEN_UNDEFINED W_TOKEN_IN W_TOKEN_ARRAY_NAME
        ...
        {   wPopStack(wBlockStack);
            $$ = wSymbolNode( W_OP_FOR_EACH, wReturnVar($3, W_SYM_VARIABLE, 0),
                0, wSymbolNode( W_OP_NOOP, $5, 0, NULL, NULL ), $8 ); }
    ...
    | W_TOKEN_INPUT W_TOKEN_STRING ';' lval sep
        {   $4->right = wOpNode( W_OP_INPUT, 0, 
                wStringNode( W_OP_STRING, $2, 0, NULL, NULL ), NULL );
            $$ = $4; }

    | W_TOKEN_INPUT lval sep
        {   $2->right = wOpNode( W_OP_INPUT, 0, NULL, NULL );
            $$ = $2; }

    | W_TOKEN_LINE W_TOKEN_INPUT fileHandle ',' lval sep
        {   /* set into lval */
            $5->right = wOpNode( W_OP_INPUT, 1, $3, NULL );
            $$ = $5; }
    ...
    | W_TOKEN_PRINT printList sep
        {   $$ = wOpNode( W_OP_PRINT, 0, NULL, $2 ); }

    | '?' printList sep
        {   $$ = wOpNode( W_OP_PRINT, 0, NULL, $2 ); }

    | W_TOKEN_PRINT '#' expr ',' printList sep
        {   $$ = wOpNode( W_OP_PRINT, 0, $3, $5 ); }
    ...
    | W_TOKEN_RETURN returnList sep
        {   if (!wInStack(wBlockStack,W_TOKEN_FUNCTION)) {
                wFatal( W_SYNTAX, "Return outside Function");
            }
            $$ = wOpNode( W_OP_RETURN_VALUE, 0, $2, NULL ); }
    ...
    | W_TOKEN_SELECT W_TOKEN_CASE
      ...
      expr sep cases sep
        {   wPopStack(wBlockStack);
            $$ = wOpNode( W_OP_CASE_SELECT, 0, $4, $6 ); }
    ...
    | lval W_TOKEN_INCR_SELF expr sep
        {   /* simple var or array? */
            ...
            case W_OP_ASSIGN:
                ...
                $1->right = wOpNode( $2, 0,
                    wSymbolNode( W_OP_VAR, $1->value.symbol, 0, NULL, NULL ), $3 );
                break;

            case W_OP_ARRAY_SET:
                /* create a node to hold the operation */
                $1->right = wOpNode( $2, 0,
                        wSymbolNode( W_OP_ARRAY_GET, $1->value.symbol,
                            0, $1->left, NULL ),
                        $3 );
                break;
...
*/
    ...
    | W_TOKEN_UNDEFINED '('
        ...
        {   wSymbol *s = wAddSymbol( $1, NULL, W_SYM_FORWARD_SUB, W_TRUE );
            ...
            $$ = wSymbolNode( W_OP_FUNCTION_CALL, s, 0, $4, NULL );
        }
...
lval:
      W_TOKEN_UNDEFINED
        {   $$ = wSymbolNode( W_OP_ASSIGN, wReturnVar($1, W_SYM_VARIABLE, 0), 0, NULL, NULL );
            wFree( $1 ); }

    | W_TOKEN_VARIABLE_NAME
        {   $$ = wSymbolNode( W_OP_ASSIGN, $1, 0, NULL, NULL ); }

    | W_TOKEN_ARRAY_NAME '[' indexList
        { wCheckArgCount( $1, wPopStack( wArgCountStack ) );
          $$ = wSymbolNode( W_OP_ARRAY_SET, $1, 0, $3, NULL ); }
...
cases:
    ...
    | W_TOKEN_CASE W_TOKEN_ELSE sep block
        { $$ = wComma(  wComma( wOpNode( W_OP_CASE_ELSE, 0, NULL, NULL ), $4 ),
                        NULL ); }
...
caseTests:
      expr W_TOKEN_TO expr caseTail
        { $$ = wOpNode( W_OP_CASE_RANGE, 0, wComma( $1, $3 ), $4 ); }

    | expr caseTail
        { $$ = wOpNode( W_OP_CASE_CMP, W_EQ, $1, $2 ); }

    | W_TOKEN_IS W_TOKEN_EQ expr caseTail
        { $$ = wOpNode( W_OP_CASE_CMP, W_EQ, $3, $4 ); }

    | W_TOKEN_IS W_TOKEN_NE expr caseTail
        { $$ = wOpNode( W_OP_CASE_CMP, W_NE, $3, $4 ); }

    | W_TOKEN_IS W_TOKEN_GT expr caseTail
        { $$ = wOpNode( W_OP_CASE_CMP, W_GT, $3, $4 ); }

    | W_TOKEN_IS W_TOKEN_LT expr caseTail
        { $$ = wOpNode( W_OP_CASE_CMP, W_LT, $3, $4 ); }

    | W_TOKEN_IS W_TOKEN_LE expr caseTail
        { $$ = wOpNode( W_OP_CASE_CMP, W_LE, $3, $4 ); }

    | W_TOKEN_IS W_TOKEN_GE expr caseTail
        { $$ = wOpNode( W_OP_CASE_CMP, W_GE, $3, $4 ); }
...
expr:
    ...
    | expr '^' optSep expr
        { $$ = wOpNode( W_OP_POWER, 0, $1, $4 ); }

    | '-' expr %prec UMinus
        { $$ = wOpNode( W_OP_NEGATE, 0, $2, NULL ); }
    ...
    | expr '*' optSep expr
        { $$ = wOpNode( W_OP_MUL, 0, $1, $4 ); }

    | expr '/' optSep expr
        { $$ = wOpNode( W_OP_DIV, 0, $1, $4 ); }

    | expr '\\' optSep expr
        { $$ = wOpNode( W_OP_IDIV, 0, $1, $4 ); }

    | expr '%' optSep expr
        { $$ = wOpNode( W_OP_MOD, 0, $1, $4 ); }

    | expr W_TOKEN_SHL optSep expr
        { $$ = wOpNode( W_OP_SHL, 0, $1, $4 ); }

    | expr W_TOKEN_SHR optSep expr
        { $$ = wOpNode( W_OP_SHR, 0, $1, $4 ); }
    ...
    | expr '+' optSep expr
        { $$ = wOpNode( W_OP_ADD, 0, $1, $4 ); }

    | expr '&' optSep expr
        { $$ = wOpNode( W_OP_CONCAT, 0, $1, $4 ); }

    | expr '-' optSep expr
        { $$ = wOpNode( W_OP_SUB, 0, $1, $4 ); }

    | expr '|' optSep expr
        { $$ = wOpNode( W_OP_OR_BITS, 0, $1, $4 ); }
    ...
    | expr W_TOKEN_EQ optSep expr
        { $$ = wOpNode( W_OP_CMP, W_EQ, $1, $4 ); }

    | expr W_TOKEN_NE optSep expr
        { $$ = wOpNode( W_OP_CMP, W_NE, $1, $4 ); }

    | expr W_TOKEN_LT optSep expr
        { $$ = wOpNode( W_OP_CMP, W_LT, $1, $4 ); }

    | expr W_TOKEN_GT optSep expr
        { $$ = wOpNode( W_OP_CMP, W_GT, $1, $4 ); }

    | expr W_TOKEN_LE optSep expr
        { $$ = wOpNode( W_OP_CMP, W_LE, $1, $4 ); }

    | expr W_TOKEN_GE optSep expr
        { $$ = wOpNode( W_OP_CMP, W_GE, $1, $4 ); }


        /* precedence 6 */

    | W_TOKEN_NOT expr
      { $$ = wOpNode( W_OP_NOT, 0, $2, NULL ); }
    ...
    | W_TOKEN_CLASS_NAME W_TOKEN_METHOD_NAME '('
        ...
        {   wPopStack( wArgCountStack );
            $$ = wStringNode( W_OP_CLASS_METHOD, $2, 0, 
                    wSymbolNode( W_OP_COMMA, $1, 0, NULL, NULL ), $5 ); }


    | W_TOKEN_CLASS_NAME W_TOKEN_METHOD_NAME
        {   $$ = wStringNode( W_OP_CLASS_METHOD, $2, 0,
                    wSymbolNode( W_OP_COMMA, $1, 0, NULL, NULL ), NULL ); }
    ...
    | expr W_TOKEN_METHOD_NAME '('
        ...
        argList
        {   wPopStack( wArgCountStack );
            $$ = wStringNode( W_OP_METHOD, $2, 0, $1, $5 ); }

    | expr W_TOKEN_METHOD_NAME
        {   $$ = wStringNode( W_OP_METHOD, $2, 0, $1, NULL ); }
    ...
    | W_TOKEN_NEW W_TOKEN_CLASS_NAME '('
        ...
        argList
        {   wPopStack( wArgCountStack );
            $$ = wSymbolNode( W_OP_NEW, $2, 0, $5, NULL ); }

    | W_TOKEN_NEW W_TOKEN_CLASS_NAME
        {   $$ = wSymbolNode( W_OP_NEW, $2, 0, NULL, NULL ); }
    ...
    | W_TOKEN_CLASS_NAME '('
        ...
        argList
        {   wPopStack( wArgCountStack );
            $$ = wSymbolNode( W_OP_NEW_TMP, $1, 0, $4, NULL ); }

    | W_TOKEN_CLASS_NAME
        {   wExpected("("); }

    | W_TOKEN_UNDEFINED
        { $$ = wSymbolNode( W_OP_VAR, wReturnVar($1, W_SYM_VARIABLE, 0), 0, NULL, NULL );
          wFree($1); }

    | W_TOKEN_VARIABLE_NAME
        { $$ = wSymbolNode( W_OP_VAR, $1, 0, NULL, NULL ); }

    | W_TOKEN_ARRAY_NAME optBrackets
        { $$ = wSymbolNode( W_OP_ARRAY_PTR, $1, 0, NULL, NULL ); }

    | W_TOKEN_ARRAY_NAME '[' indexList
        { int args = wPopStack( wArgCountStack );
          wCheckArgCount( $1, args );
          $$ = wSymbolNode( W_OP_ARRAY_GET, $1, 0, $3, NULL ); }

    | W_TOKEN_ARRAY_NAME
        {   wExpected("'['"); }

    | W_TOKEN_CONSTANT_NAME
        {   $$ = wSymbolNode( W_OP_CONST_GET, $1, 0, NULL, NULL ); }

    | W_TOKEN_INTEGER
        { $$ = wIntNode( W_OP_INT, $1, 0, NULL, NULL ); }

    | W_TOKEN_FLOAT
        { $$ = wFloatNode( W_OP_FLOAT, $1, 0, NULL, NULL ); }

    | W_TOKEN_STRING
        { $$ = wStringNode( W_OP_STRING, $1, 0, NULL, NULL ); }
    ...
    | W_TOKEN_BUILTIN_NAME '('
        ...
        { int args = wPopStack( wArgCountStack );
          wCheckArgCount( $1, args );
          $$ = wSymbolNode( W_OP_BUILTIN, $1, 0, $4, NULL ); }

    | W_TOKEN_BUILTIN_NAME
        { wCheckArgCount( $1, 0 );
          $$ = wSymbolNode( W_OP_BUILTIN, $1, 0, NULL, NULL ); }
    ...
    | W_TOKEN_FUNCTION_NAME '('
        ...
        {   int args = wPopStack( wArgCountStack );
            wCheckArgCount( $1, args );
            $$ = wSymbolNode( W_OP_FUNCTION, $1, 0, $4, NULL ); }

    | W_TOKEN_FUNCTION_NAME
        {   wCheckArgCount( $1, 0 );
            $$ = wSymbolNode( W_OP_FUNCTION, $1, 0, NULL, NULL ); }
    ...
    | W_TOKEN_UNDEFINED '('
        ...
        {   wSymbol *s = wAddSymbol( $1, NULL, W_SYM_FORWARD_FUNCTION, W_TRUE );
            ...
            $$ = wSymbolNode( W_OP_FUNCTION, s, 0, $4, NULL );
        }
...
loopTail:
      W_TOKEN_EQ expr W_TOKEN_TO expr W_TOKEN_STEP expr sep block 
        { $$ = wSymbolNode( W_OP_FOR_STEP, NULL, 0,
                    wComma( $2, wComma($4, $6) ),
                    $8 ); }
    ...
    | W_TOKEN_EQ expr W_TOKEN_TO expr sep block 
        { $$ = wSymbolNode( W_OP_FOR, NULL, 0, wComma( $2, $4 ), $6 ); }
...
printList:
    ...
    | ',' printList
        { $$ = wOpNode( W_OP_PRINT_VALUE, W_PRINT_TAB, NULL, $2 ); }

    | expr ',' printList
        { $$ = wOpNode( W_OP_PRINT_VALUE, W_PRINT_TAB, $1, $3 ); }

    | expr ';' printList
        { $$ = wOpNode( W_OP_PRINT_VALUE, W_PRINT, $1, $3 ); }

    | expr
        { $$ = wOpNode( W_OP_PRINT_VALUE, W_PRINT_NEWLINE, $1, NULL ); }
...
indexList:
      ...
      indexListTail
        { int args = wPeekStack( wArgCountStack );
          $$ = wComma( $2, wIntNode( W_OP_INT, args, 0, NULL, NULL ) ); }
    ;
...
dimItem:
      W_TOKEN_UNDEFINED '[' ']' optArrayValueList
        { wSymbol *s = wCreateArraySymbol( $1, 0 );
          $$ = wSymbolNode( W_OP_INIT_DYNAMIC_ARRAY, s, 0, $4, NULL );
          wFree( $1 ); }

    | W_TOKEN_UNDEFINED '[' dimIndexList
        { int args = wPopStack( wArgCountStack );
          wSymbol *s = wCreateArraySymbol( $1, args );
          $$ = wSymbolNode( W_OP_INIT_STATIC_ARRAY, s, 0, $3, NULL );
          wFree( $1 ); }

    | W_TOKEN_VARIABLE_NAME '[' ']' optArrayValueList
    ...
    | W_TOKEN_ARRAY_NAME '[' ']' optArrayValueList
        { wSymbol *s = wCreateArraySymbol( $1->name, 0 );
          $$ = wSymbolNode( W_OP_INIT_DYNAMIC_ARRAY, s, 0, $4, NULL ); }
    ...
    | W_TOKEN_UNDEFINED W_TOKEN_EQ expr
        {   $$ = wSymbolNode( W_OP_ASSIGN, wReturnVar($1, W_SYM_VARIABLE, 1), 0, NULL, $3 );
            wFree( $1 ); }
    ...
    | W_TOKEN_VARIABLE_NAME W_TOKEN_EQ expr
        {   $$ = wSymbolNode( W_OP_ASSIGN, wReturnVar($1->name, W_SYM_VARIABLE, 1), 0, NULL, $3 ); }
...
dimIndexItems:
      expr ']'
        { wIncrStack( wArgCountStack, 1 );
          $$ = wComma( $1, wIntNode( W_OP_INT, 0, 0, NULL, NULL) ); }
    ...
    | expr ',' optSep dimIndexItems
        { wIncrStack( wArgCountStack, 1 );
          $$ = wComma( $4, wComma( $1, wIntNode( W_OP_INT, 0, 0, NULL, NULL)) ); }
    ;
...
optArrayValueList:
    ...
    | W_TOKEN_EQ '{' optSep arrayValueList
        { $$ = $4; }
	
    | W_TOKEN_EQ error
	{ wFatal( W_SYNTAX, "Expected a '{'" ); }
...
arrayValueList:
    ...
    | expr ',' optSep arrayValueList
	{ $$ = wComma( $1, $4 ); }
	
    | expr error
	{ wFatal( W_SYNTAX, "Expected a ',' or '}'" ); }
...
constItem:
      W_TOKEN_UNDEFINED W_TOKEN_EQ expr
        {   $$ = wSymbolNode( W_OP_CONST_Set, wCreateConst( $1 ), 0, $3, NULL );
            wFree( $1 ); }
