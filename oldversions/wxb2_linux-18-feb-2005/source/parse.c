...
/* PENDING ROUTINES */
wCode *wParseStatementDestroy()        { wErrorThrow( W_ERROR_SYNTAX, "Destroy not yet coded"); return NULL; }
...
wCode *wParseDotMethod()
{
    ...
    if (wParseTokenIs(W_TOKEN_ME) || wParseTokenIs(W_TOKEN_MYBASE)) {
...
wCode *wParseStatementClass()
{
    ...
    wCode *code;
    wSymbol *klass, *inherits, *property;
...
/* build jump clause at head of static assignment */
int wParseStaticTest( wCode *code, int here )
{
    /* first assignment? */
    if (here == -1) {
...
{
    int indexCount, symbolStartIndex, symbolEndIndex, here, scope;
    wCode *code, *tmpCode;
    wSymbol *s;
    ...
    if (isStatic) {
        ...
        if (wTheCurrentScope == wTheGlobalScope) {
            wErrorThrow( W_ERROR_SYNTAX,
                "Static variables not allowed outside routines" );
        }
    }
    ...
    while (1) {
        ...
        ||  wParseTokenIs(W_TOKEN_UNDEFINED)) {
            if (isStatic) {
                ...
                wTheCurrentScope = wTheGlobalScope;
                /* FIXME: give a real name for debugging */
                s = wSymbolNewLocal( "_static", 0 );
                wTheCurrentScope = scope;

                /* create an aliased variable to point to the global */
                s = wSymbolNewLocal( wTheToken.text, s->index );
        ...
        if (wParseTokenIs('[')) {
            ...
            if (wParseTokenIs(W_TOKEN_EQ)) {
                ...
                /* add test if static assignment */
                if (isStatic) {
                    here = wParseStaticTest( code, here );
                }
...
wCode *wParseStatementRoutineName( int returnCount )
{
    ...
    /* method and option strict? */
    if (s->scope == wTheClassScope && wTheOptionStrict) {
        wErrorThrow( W_ERROR_SYNTAX,  "Expected \"Me.%s\", not \"%s\"", s->name, s->name );
    }
...
wCode *wParseStatement()
{
    ...
    if (wParseTokenIs('~')) {
        ...
        code = wParseStatementDim( W_FALSE );
        ...
        code = wParseStatementDim( W_TRUE );
