...
void wVmExecute( wSymbol *sRoutine )
{
    ...
    int priorTraceDown, priorTraceOn, traceUp;
    wNumber n1, n2, n3;
    ...
    while (!wTheHaltFlag) {
        ...
        /* wait for debugging? */
        while ((wTheTraceOnFlag) &&
            !wTheTraceGotoNextBreak &&
            wTheTraceWaitFlag &&
            !wTheHaltFlag) {
#ifdef __WXWIN__
            /* don't hog the CPU */
        ...
        if (wTheTraceUpFlag) {
            ...
            /* clear the flag */
            wTheTraceUpFlag = W_FALSE;

            /* restore trace on exit of this loop */
            traceUp = W_TRUE;
        }
        ...
        case W_OP_CALL:
            ...
            /* save the trace state */
            priorTraceOn = wTheTraceOnFlag;
            priorTraceDown = wTheTraceDownFlag;

            /* set new trace state */
            wTheTraceOnFlag = wTheTraceDownFlag;

            wCallRoutine( s, arg2, arg3, NULL, NULL );

            /* restore the trace state */
            wTheTraceOnFlag |= priorTraceOn;
            wTheTraceDownFlag = priorTraceDown;
        ...
        case W_OP_HANDLESID:
            ...
            /* routine is a parameter */
            arg1 = pcode[++pc];
            s = wSymbolLookup( arg1 );

            if (opcode == W_OP_HANDLESID) {
            ...
            /* name of event from stack */
            v1 = wStackPopString();
            n3 = (int)wEventNameToType( wStringChar(v1) );
			wVariantDeref( v1 );

            /* pop the object's handle off the stack */
            handle = wStackPopObjectPtr( NULL );

            /* connect */
            wConnect( n1, n2, handle, n3, s );
#else
            wErrorThrow( W_ERROR_INTERNAL,
                "Handles is not available in console mode");
#endif
        ...
        case W_OP_SHL:
			...
			/* left bitshift */
            wStackPushNumber( (int)n2 << (int)n1) ;
        ...
        case W_OP_TRACE:
            ...
            /* trace? */
            if (wTheTraceOnFlag && ! wTheTraceGotoNextBreak) {
                wTrace( arg1 );
            }
            break;
        ...
        case W_OP_MOD_SET:
            ...
            /* get the number to  add/subtract, etc. */
            n1 = wStackPopNumber();

            /* need to convert to a number */
            if (lval->dataType != W_TYPE_NUMBER) {
            ...
            switch (modifier) {
                ...
                case W_OP_IDIV_SET:
                    ...
                    break;
                case W_OP_MOD_SET:
                    lval->value.number = ( (long)lval->value.number % (long)n1 );
                    break;
            }

            /* coerce back to the proper type */
            wVmTypeCast( lval );
...
/* builds decompiled opcode in string, returns new pc position */
int wVmDecompileOp( wSymbol *sRoutine, int *pcode, int pc )
{
    ...
    case W_OP_JSR:
        arg1 = pcode[++pc];
        wVmOpFormat( at, opcode, 1, arg1, 0, 0, "to %d", arg1+pc );
        break;
    ...
    case W_OP_INTEGER:
        pc++;
        arg1 = pcode[pc];
        wVmOpFormat( at, opcode, 1, arg1, 0, 0, "%d", arg1);
        break;
     ...
     case W_OP_LITERAL:
        ...
        wVmOpFormat( at, opcode, 1, arg1, 0, 0, "%s", buffer );
        wFree( buffer );
        break;
    ...
    case W_OP_PROP:
        ...
        wVmOpFormat( at, opcode, 1, arg1, 0, 0,
    ...
    case W_OP_FORSTEP1:
        arg1 = pcode[++pc];
        arg2 = pcode[++pc];
        wVmOpFormat( at, opcode, 2, arg1, arg2, 0,
            "%s, %s",
    ...
    case W_OP_FOREACHLOOP:
        ...
        if (arg3) {
            /* show 3 ops */
            wVmOpFormat( at, opcode, 3, arg1, arg2, arg3,
                "%s, %s, %s",
                wVmVariableName( sRoutine, arg1 ),
                wVmVariableName( sRoutine, arg2 ),
    ...
    case W_OP_CREATEARRAY:
        arg1 = pcode[++pc];
        wVmOpFormat( at, opcode, 1, arg1, 0, 0, "%d indexes", arg1 );
        break;
    ...
    case W_OP_CREATELIST:
        arg1 = pcode[++pc];
        wVmOpFormat( at, opcode, 1, arg1, 0, 0, "%d elements", arg1 );
        break;
    ...
    case W_OP_SETTYPE:
        ...
        arg1 = pcode[++pc];
        arg2 = pcode[++pc];
        wVmOpFormat( at, opcode, 1, arg1, arg2, 0, 
            "%s must be %s",
            wVmVariableName( sRoutine, arg1 ),
...
void wVmOpFormat( int pc, int opcode, int args, int arg1, int arg2, int arg3, char *commentFmt, ... )
{
    ...
    /* need to allocate a comment buffer? */
    if (!wTheOpComment) {
        wTheOpComment = wCharMalloc( W_INBUFFER_MAX );
    }

    /* format comment */
    va_start(argList, commentFmt);
    vsprintf( wTheOpComment, commentFmt, argList );
    va_end(argList);
