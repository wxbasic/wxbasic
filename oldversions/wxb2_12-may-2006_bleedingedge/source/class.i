...
%class wxValidator
//%class wxWindowList
%if(win) %class wxMetafile
        %include "wx/metafile.h"
%if(win) %class wxBitmapHandler
%if(win) %class wxGDIImageHandler
%class wxMask
%class wxProcess
...
%}
%builtin %rename wxDisplaySize void DisplaySize()
...
%ctor wxFileById(int fd)
static bool Access(const char * name, wxFile::OpenMode mode)
void Attach(int fd)
void Close()
...
bool DirExists()
// FIXME: alias
//bool DirExists(const wxString& dir)
...
// FIXME: string arrays not supported yet
...
// FIXME: wxChar not supported yet
...
bool SetCwd()
// FIXME: alias
%rename SetCwdString static bool SetCwd(const wxString& cwd)
...
// FIXME: these need wrappers, because they return values back via pointers
...
// enum wxPathNormalize
%num wxPATH_NORM_ENV_VARS
%num wxPATH_NORM_DOTS
%num wxPATH_NORM_TILDE
%num wxPATH_NORM_CASE
%num wxPATH_NORM_ABSOLUTE
%num wxPATH_NORM_LONG
%num wxPATH_NORM_ALL

// for GetPath
%num wxPATH_GET_VOLUME
%num wxPATH_GET_SEPARATOR
%num wxPATH_MKDIR_FULL
...
%ctor wxRectFromPoints(const wxPoint& topLeft, const wxPoint& bottomRight)
%ctor wxRectFromPointAndSize(const wxPoint& pos, const wxSize& size)
...
%rename DeflateDiff void Deflate(wxCoord diff)
%rename DeflateDxDy wxRect Deflate(wxCoord dx, wxCoord dy) const
...
%rename InflateDiff void Inflate(wxCoord diff)
%rename InflateDxDy wxRect Inflate(wxCoord dx, wxCoord dy) const
bool Inside(int x, int y) const
bool Intersects(const wxRect& rect) const
%rename OffsetDxDy void Offset(wxCoord dx, wxCoord dy)
%rename OffsetPoint void Offset(const wxPoint& pt)
...
int GetJoin() const
%if(win) wxBitmap* GetStipple() const
int GetStyle() const
int GetWidth() const
bool Ok() const
...
%if(win) %ctor wxPenFromBitmap(const wxBitmap& stipple, int width)
...
// FIX: not in GTK
%if(win) %class wxMetafileDC, wxDC 
%include "wx/metafile.h"
...
// FIXME: can't create - abstract class
...
%{
    void wxPrintout_GetPageInfo()
    {
        int minPage, maxPage, pageFrom, pageTo;
        ((wxPrintout *)wTheCall->self->ptr)->GetPageInfo(&minPage, &maxPage, &pageFrom, &pageTo);
		wWrapDerefArgs();
        wStackPushNumber( (wNumber)minPage );
        wStackPushNumber( (wNumber)maxPage );
        wStackPushNumber( (wNumber)pageFrom );
        wStackPushNumber( (wNumber)pageTo );
    }

    void wxPrintout_GetPageSizeMM()
    {
        int w, h;
        ((wxPrintout *)wTheCall->self->ptr)->GetPageSizeMM(&w, &h);
		wWrapDerefArgs();
        wStackPushNumber( (wNumber)w );
        wStackPushNumber( (wNumber)h );
    }

    void wxPrintout_GetPageSizePixels()
    {
        int w, h;
        ((wxPrintout *)wTheCall->self->ptr)->GetPageSizePixels(&w, &h);
		wWrapDerefArgs();
        wStackPushNumber( (wNumber)w );
        wStackPushNumber( (wNumber)h );
    }

    void wxPrintout_GetPPIPrinter()
    {
        int w, h;
        ((wxPrintout *)wTheCall->self->ptr)->GetPPIPrinter(&w, &h);
		wWrapDerefArgs();
        wStackPushNumber( (wNumber)w );
        wStackPushNumber( (wNumber)h );
    }

    void wxPrintout_GetPPIScreen()
    {
        int w, h;
        ((wxPrintout *)wTheCall->self->ptr)->GetPPIScreen(&w, &h);
		wWrapDerefArgs();
        wStackPushNumber( (wNumber)w );
        wStackPushNumber( (wNumber)h );
    }
...
%method GetPageInfo         wxPrintout_GetPageInfo()
%method GetPageSizeMM 		wxPrintout_GetPageSizeMM()
%method GetPageSizePixels 	wxPrintout_GetPageSizePixels()
%method GetPPIPrinter 		wxPrintout_GetPPIPrinter()
%method GetPPIScreen 		wxPrintout_GetPPIScreen()
...
%num wxPRINTER_NO_ERROR
%num wxPRINTER_CANCELLED
%num wxPRINTER_ERROR
...
%if(false) %class wxPostScriptDC, wxDC
%include "wx/dcps.h"
...
%if(win) wxBitmap(void* data, int type, int width, int height, int depth = -1)
...
%if(win) static void AddHandler(wxBitmapHandler* handler)
%if(win) static void CleanUpHandlers()
...
int GetDepth() const
// FIX: Missing from GTK
%if(win) static wxList& GetHandlers()
int GetHeight() const
...
// FIX: missing in GTK
%if(win) static void InitStandardHandlers()
%if(win) static void InsertHandler(wxBitmapHandler* handler)
bool LoadFile(const wxString& name, long type)
bool Ok() const
// FIX: Missing in GTK
%if(win) static bool RemoveHandler(const wxString& name)
...
// void SetOk(int isOk)
// FIX: Missing in GTK
%if(win) void SetPalette(const wxPalette& palette)
void SetWidth(int width)
...
bool RemoveAll()
%if(win) bool Replace(int index, const wxBitmap& bitmap, const wxBitmap& mask = wxNullBitmap)
...
%ctor wxCursorFromId(int cursorId)
%ctor wxStockCursor(int cursorId)
%ctor wxCursorFromImage(const wxImage& image)
%ctor wxCursorCopy(const wxCursor& cursor)
...
// FIX: Missing in GTK
%if(win) void SetAuto3D(const bool auto3D)
...
// FIX: Missing in GTK
%if(win) virtual void DragAcceptFiles(bool accept)
...
%rename SetSizeXY virtual void SetSize(int width, int height)
...
virtual void SetTitle(const wxString& title)
// FIX: Missing in GTK
%if(win) bool ShowFullScreen(bool show, long style = wxFULLSCREEN_ALL)
...
// FIX: Missing in GTK
%if(win) wxMenu* GetWindowMenu() const
// virtual wxMDIClientWindow* OnCreateClient()
// Decl error
// virtual void SetToolBar(wxWindow* toolbar)
virtual void SetToolBar(wxToolBar* toolbar)
// FIX: Missing in GTK
%if(win) void SetWindowMenu(wxMenu* menu)
void Tile()
...
%{
    void wxScrolledWindow_CalcScrolledPosition()
    {
        ...
        wWrapDerefArgs();
        wStackPushNumber( (wNumber)xx );
        wStackPushNumber( (wNumber)yy );
    }
%}
%method CalcScrolledPosition wxScrolledWindow_CalcScrolledPosition( int x, int y )
...
%}
%method CalcUnscrolledPosition wxScrolledWindow_CalcUnscrolledPosition( int x, int y )
...
%if(win) %builtin %rename wxDateTime_GetMonthName static wxString wxDateTime::GetMonthName(int month, wxDateTime::NameFlags flags = wxDateTime::Name_Full)
...
%num wxLIST_ALIGN_DEFAULT
%num wxLIST_ALIGN_LEFT
%num wxLIST_ALIGN_TOP
%num wxLIST_ALIGN_SNAP_TO_GRID

%num wxIMAGE_LIST_NORMAL
%num wxIMAGE_LIST_SMALL
%num wxIMAGE_LIST_STATE

%num wxLIST_NEXT_ABOVE
%num wxLIST_NEXT_ALL
%num wxLIST_NEXT_BELOW
%num wxLIST_NEXT_LEFT
%num wxLIST_NEXT_RIGHT

%num wxLIST_HITTEST_ABOVE
%num wxLIST_HITTEST_BELOW
%num wxLIST_HITTEST_NOWHERE
%num wxLIST_HITTEST_ONITEMICON
%num wxLIST_HITTEST_ONITEMLABEL
%num wxLIST_HITTEST_ONITEMRIGHT
%num wxLIST_HITTEST_ONITEMSTATEICON
%num wxLIST_HITTEST_TOLEFT
%num wxLIST_HITTEST_TORIGHT
%num wxLIST_HITTEST_ONITEM
...
// FIX: Missing in GTK
%if(win) void EndEditLabel(bool cancelEdit)
void EnsureVisible(const wxTreeItemId& item)
void Expand(const wxTreeItemId& item)
// FIX: Missing in GTK
%if(win) bool GetBoundingRect(const wxTreeItemId& item, wxRect& rect, bool textOnly = FALSE) const
...
%if(win) void SetMargins(const wxSize& size)
...
%if(false) %class wxPlotWindow, wxScrolledWindow
...
// FIX: Missing in GTK
%if(win) %alias GetBackgroundColor wxColour& GetBackgroundColour() const
// Fix: Cannot convert 'const wxBitmap *' to 'wxBitmap *'
// wxBitmap& GetBitmap(bool checked = TRUE) const
// FIX: Missing in GTK
%if(win) wxFont& GetFont() const
wxString GetHelp() const
int GetId() const
wxString GetLabel() const
static wxString GetLabelFromText(const wxString& text)
// FIX: Missing in GTK
%if(win) int GetMarginWidth() const
// Fix: Member is ambiguous: 'wxMenuItemBase::GetName' and 'wxOwnerDrawn::GetName'
// wxString GetName() const
wxString GetText() const
wxMenu* GetSubMenu() const
// FIX: Missing in GTK
%if(win) %alias GetTextColor wxColour& GetTextColour() const
bool IsCheckable() const
bool IsChecked() const
bool IsEnabled() const
bool IsSeparator() const
// FIX: Missing in GTK
%if(win) %alias SetBackgroundColor void SetBackgroundColour(const wxColour& colour) const
void SetBitmap(const wxBitmap& bitmap) const
%if(win) void SetBitmaps(const wxBitmap& checked, const wxBitmap& unchecked = wxNullBitmap) const
%if(win) void SetFont(const wxFont& font) const
void SetHelp(const wxString& helpString) const
%if(win) void SetMarginWidth(int width) const
// Fix: Member is ambiguous: 'wxMenuItemBase::SetName' and 'wxOwnerDrawn::SetName'
// void SetName(const wxString& text) const
// FIX: Missing in GTK
%if(win) %alias SetTextColor void SetTextColour(const wxColour& colour) const


%num wxITEM_NORMAL
%num wxITEM_SEPARATOR
%num wxITEM_CHECK
%num wxITEM_RADIO
...
%event wxEVT_NC_PAINT
%event wxEVT_PALETTE_CHANGED
%event wxEVT_CLOSE_WINDOW
%event wxEVT_CHAR
%event wxEVT_MEASURE_ITEM
%event wxEVT_POWER
%event wxEVT_COMPARE_ITEM
%event wxEVT_QUERY_END_SESSION
%event wxEVT_NAVIGATION_KEY
%event wxEVT_PAINT_ICON
%event wxEVT_SETTING_CHANGED
%event wxEVT_MAXIMIZE
%event wxEVT_DRAW_ITEM
%event wxEVT_CREATE
%event wxEVT_SHOW
%event wxEVT_MOUSE_CAPTURE_CHANGED
%event wxEVT_END_SESSION
%event wxEVT_QUERY_NEW_PALETTE
...
%event wxEVT_COMMAND_RIGHT_DCLICK
%event wxEVT_COMMAND_LISTBOX_DOUBLECLICKED
%event wxEVT_COMMAND_LEFT_CLICK
%event wxEVT_COMMAND_SPINCTRL_UPDATED
%event wxEVT_COMMAND_VLBOX_SELECTED
%event wxEVT_COMMAND_TEXT_UPDATED
%event wxEVT_COMMAND_BUTTON_CLICKED
%event wxEVT_COMMAND_CHECKLISTBOX_TOGGLED
%event wxEVT_COMMAND_RADIOBUTTON_SELECTED
%event wxEVT_COMMAND_RIGHT_CLICK
%event wxEVT_COMMAND_LISTBOX_SELECTED
%event wxEVT_COMMAND_KILL_FOCUS
%event wxEVT_COMMAND_CHECKBOX_CLICKED
%event wxEVT_COMMAND_SET_FOCUS
%event wxEVT_COMMAND_RADIOBOX_SELECTED
%event wxEVT_COMMAND_TOOL_ENTER
%event wxEVT_COMMAND_SCROLLBAR_UPDATED
...
%event wxEVT_COMMAND_ENTER
%event wxEVT_COMMAND_LEFT_DCLICK
%event wxEVT_COMMAND_TOOL_RCLICKED
%event wxEVT_COMMAND_FIND
%event wxEVT_COMMAND_FIND_NEXT
%event wxEVT_COMMAND_FIND_REPLACE
%event wxEVT_COMMAND_FIND_REPLACE_ALL
%event wxEVT_COMMAND_FIND_CLOSE
...
// FIXME:
wxHelpEvent(WXTYPE eventType = 0, wxWindowID id = 0, const wxPoint& pt = wxDefaultPosition)
const wxPoint& GetPosition() const
void SetPosition(const wxPoint& pt)

%event wxEVT_HELP
%event wxEVT_DETAILED_HELP
...
%event wxEVT_CALENDAR_MONTH_CHANGED
%event wxEVT_CALENDAR_DAY_CHANGED
%event wxEVT_CALENDAR_YEAR_CHANGED
%event wxEVT_CALENDAR_WEEKDAY_CLICKED
%event wxEVT_CALENDAR_DOUBLECLICKED
%event wxEVT_CALENDAR_SEL_CHANGED
...
// not portable
//%event wxEVT_DIALUP_DISCONNECTED
//%event wxEVT_DIALUP_CONNECTED
...
%event wxEVT_COMMAND_LIST_BEGIN_RDRAG
%event wxEVT_COMMAND_LIST_ITEM_MIDDLE_CLICK
%event wxEVT_COMMAND_LIST_INSERT_ITEM
%event wxEVT_COMMAND_LIST_ITEM_DESELECTED
%event wxEVT_COMMAND_LIST_KEY_DOWN
%event wxEVT_COMMAND_LIST_DELETE_ALL_ITEMS
%event wxEVT_COMMAND_LIST_BEGIN_LABEL_EDIT
%event wxEVT_COMMAND_LIST_END_LABEL_EDIT
%event wxEVT_COMMAND_LIST_ITEM_SELECTED
%event wxEVT_COMMAND_LIST_GET_INFO
%event wxEVT_COMMAND_LIST_DELETE_ITEM
%event wxEVT_COMMAND_LIST_SET_INFO
%event wxEVT_COMMAND_LIST_BEGIN_DRAG
%event wxEVT_COMMAND_LIST_ITEM_ACTIVATED
%event wxEVT_COMMAND_LIST_COL_CLICK
%event wxEVT_COMMAND_LIST_ITEM_RIGHT_CLICK
...
%event wxEVT_NC_MIDDLE_UP
%event wxEVT_RIGHT_DOWN
%event wxEVT_NC_RIGHT_DOWN
%event wxEVT_NC_RIGHT_UP
%event wxEVT_KILL_FOCUS
%event wxEVT_MOTION
%event wxEVT_NC_LEFT_DOWN
%event wxEVT_NC_MOTION
%event wxEVT_NC_LEAVE_WINDOW
%event wxEVT_MIDDLE_DOWN
%event wxEVT_LEFT_DCLICK
%event wxEVT_NC_ENTER_WINDOW
%event wxEVT_MIDDLE_UP
%event wxEVT_NC_MIDDLE_DOWN
%event wxEVT_MIDDLE_DCLICK
%event wxEVT_NC_RIGHT_DCLICK
%event wxEVT_NC_LEFT_DCLICK
%event wxEVT_SET_FOCUS
%event wxEVT_LEAVE_WINDOW
%event wxEVT_NC_LEFT_UP
%event wxEVT_ENTER_WINDOW
%event wxEVT_LEFT_UP
%event wxEVT_NC_MIDDLE_DCLICK
...
%event wxEVT_SCROLL_LINEUP
%event wxEVT_SCROLL_PAGEUP
%event wxEVT_SCROLL_PAGEDOWN
%event wxEVT_SCROLL_LINEDOWN
%event wxEVT_SCROLL_BOTTOM
...
%event wxEVT_SCROLLWIN_TOP
%event wxEVT_SCROLLWIN_LINEUP
%event wxEVT_SCROLLWIN_PAGEUP
%event wxEVT_SCROLLWIN_THUMBRELEASE
%event wxEVT_SCROLLWIN_BOTTOM
%event wxEVT_SCROLLWIN_PAGEDOWN
%event wxEVT_SCROLLWIN_THUMBTRACK
%event wxEVT_SCROLLWIN_LINEDOWN
...
%if(win) %class wxTabEvent, wxCommandEvent
%include "wx/event.h"
...
%event wxEVT_COMMAND_TREE_BEGIN_RDRAG
%event wxEVT_COMMAND_TREE_KEY_DOWN
%event wxEVT_COMMAND_TREE_ITEM_ACTIVATED
%event wxEVT_COMMAND_TREE_GET_INFO
%event wxEVT_COMMAND_TREE_ITEM_EXPANDING
%event wxEVT_COMMAND_TREE_SEL_CHANGED
%event wxEVT_COMMAND_TREE_ITEM_COLLAPSED
%event wxEVT_COMMAND_TREE_SET_INFO
%event wxEVT_COMMAND_TREE_BEGIN_LABEL_EDIT
%event wxEVT_COMMAND_TREE_ITEM_EXPANDED
%event wxEVT_COMMAND_TREE_SEL_CHANGING
%event wxEVT_COMMAND_TREE_ITEM_COLLAPSING
%event wxEVT_COMMAND_TREE_BEGIN_DRAG
%event wxEVT_COMMAND_TREE_END_DRAG
%event wxEVT_COMMAND_TREE_DELETE_ITEM
%event wxEVT_COMMAND_TREE_ITEM_MIDDLE_CLICK
%event wxEVT_COMMAND_TREE_ITEM_RIGHT_CLICK
%event wxEVT_COMMAND_TREE_END_LABEL_EDIT
...
%rename AddBookFileName bool AddBook(const wxFileName& book_file, bool show_wait_msg)
...
%num wxDIRCTRL_DIR_ONLY
%num wxDIRCTRL_3D_INTERNAL
%num wxDIRCTRL_SELECT_FIRST
%num wxDIRCTRL_SHOW_FILTERS
%num wxDIRCTRL_EDIT_LABELS
...
%if(false) %class wxWave, wxObject
%include "wx/wave.h"
...
bool SetAscii()
bool SetBinary()
// FIXME: added enum to fix problem with casting
